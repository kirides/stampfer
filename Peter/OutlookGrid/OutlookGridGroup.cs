    ...
    public class OutlookgGridDefaultGroup : IOutlookGridGroup
    {
        ...
        public virtual string Text
        {
            get
            {
                ...
                {
                    var grid = (OutlookGrid)this.column.DataGridView;
                    if (grid.ShowItemCount)
                        return string.Format(/*"{0}: */"{0} ({1})", /*column.HeaderText, */Value.ToString(), itemCount == 1 ? "1 item" : itemCount.ToString() + " items");
                    else
