...
namespace Peter.CocoParser
{
    ...
    public class Node
    {
        ...
        private static int Ptr(Node p, bool up)
        {
            if (p == null) return 0;
        ...
        private static string Pos(Position pos)
        {
        ...
        public static void PrintNodes()
        {
            ...
            foreach (Node p in nodes)
            {
                ...
                {
                    var c = (CharClass)CharClass.classes[p.val];
    ...
    public class Graph
    {
        private static Node dummyNode = new Node(Node.eps, null, 0);
        ...
        public static void MakeSequence(Graph g1, Graph g2)
        {
            ...
            while (p != null)
            {  // link substructure
                var q = p.next; p.next = g2.l; p.up = true;
                p = q;
            }
            g1.r = g2.r;
        }
        ...
        public static void MakeIteration(Graph g)
        {
            ...
            while (p != null)
            {
                var q = p.next; p.next = g.l; p.up = true;
                p = q;
            }
        }
        ...
        {
            var s = DFA.Unescape(str.Substring(1, str.Length - 2));
    ...
    public class CharClass
    {
        ...
        private static string Ch(int ch)
        {
            if (ch < ' ' || ch >= 127 || ch == '\'' || ch == '\\') return ch.ToString();
            else return string.Format("'{0}'", (char)ch);
        }

        private static void WriteCharSet(TextWriter w, BitArray s)
        {
            int i = 0, len = s.Count;
            while (i < len)
            {
    ...
    public class Tab
    {
        ...
        private static BitArray visited;                     // mark list for graph traversals
        private static Symbol curSy;                             // current symbol in computation of sets

        //---------------------------------------------------------------------
        //  Symbol set computations
        //---------------------------------------------------------------------

        private static BitArray First0(Node p, BitArray mark)
        {
            var fs = new BitArray(Symbol.terminals.Count);
        ...
        {
            var fs = First0(p, new BitArray(Node.nodes.Count));
            if (ddt[3])
            {
                trace.WriteLine();
        ...
        private static void CompFirstSets()
        {
        ...
        private static void CompFollow(Node p)
        {
        ...
        private static void CompFollowSets()
        {
            foreach (Symbol sym in Symbol.nonterminals)
            {
        ...
        private static Node LeadingAny(Node p)
        {
        ...
        private static void FindAS(Node p)
        { // find ANY sets
            Node a;
            while (p != null)
            {
                ...
                {
                    var s1 = new BitArray(Symbol.terminals.Count);
                    var q = p;
                    while (q != null)
                    {
                        FindAS(q.sub);
        ...
        private static void CompAnySets()
        {
        ...
        private static void CompSync(Node p)
        {
            while (p != null && !visited[p.n])
            {
                visited[p.n] = true;
        ...
        private static void CompSyncSets()
        {
        ...
        private static void GetSingles(Node p, ArrayList singles)
        {
        ...
        private static void LL1Error(int cond, Symbol sym)
        {
        ...
        private static void CheckOverlap(BitArray s1, BitArray s2, int cond)
        {
            foreach (Symbol sym in Symbol.terminals)
            {
        ...
        private static void CheckAlts(Node p)
        {
            BitArray s1, s2;
            while (p != null)
            {
        ...
        private static void MarkReachedNts(Node p)
        {
            while (p != null)
            {
        ...
        private static bool IsTerm(Node p, BitArray mark)
        ...
        public static bool AllNtToTerm()
        {
            ...
            var mark = new BitArray(Symbol.nonterminals.Count);
        ...
        {
            var tab = new SortedList();
            // collect lines where symbols have been defined
            foreach (Symbol sym in Symbol.nonterminals)
            {
                var list = (ArrayList)tab[sym];
