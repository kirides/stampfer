...
namespace Peter.CocoParser
{
    public class Parser
    {
        ...
        private static bool StartOf(int s)
        {
            return set[s, t.kind];
        }
        ...
        private static bool WeakSeparator(int n, int syFol, int repFol)
        {
        ...
        private static void Coco()
        {
            ...
            Expect(6);
            var gramLine = token.line;
            genScanner = true;
            var ok = true;
            Tab.ignored = null;
            Expect(1);
            var gramName = token.val;
            var beg = t.pos;
            while (StartOf(1))
            {
                Get();
            ...
            while (t.kind == 1)
            {
                ...
                var noAttrs = sym.attrPos == null; sym.attrPos = null;
        ...
        private static void UsingDecl(out Position pos)
        {
            Expect(37);
            var beg = token.pos;
            while (StartOf(5))
        ...
        private static void SemText(out Position pos)
        {
            Expect(35);
            var beg = t.pos; var col = t.col;
            while (StartOf(9))
        ...
        private static void Expression(out Graph g)
        {
            Graph g2;
            Term(out g);
            var first = true;
        ...
        private static void TokenExpr(out Graph g)
        {
            Graph g2;
            TokenTerm(out g);
            var first = true;
        ...
        private static void Set(out BitArray s)
        {
            BitArray s2;
            SimSet(out s);
        ...
        private static void SimSet(out BitArray s)
        {
            int n1, n2;
            ...
            {
                Get();
                var c = CharClass.Find(token.val);
            ...
            else if (t.kind == 3)
            {
                ...
                foreach (var ch in name) s[ch] = true;
            }
            else if (t.kind == 5)
            {
                Char(out n1);
        ...
        private static void Char(out int n)
        {
            Expect(5);
            var name = token.val;
            name = DFA.Unescape(name.Substring(1, name.Length - 2));
            var max = CharClass.charSetSize;
            if (name.Length != 1 || name[0] > max - 1) SemErr(2);
            n = name[0] % max;

        }

        private static void Sym(out string name, out int kind)
        {
            name = "???"; kind = id;
            if (t.kind == 1)
            {
        ...
        private static void Term(out Graph g)
        {
            Graph g2;
            g = new Graph();
            if (StartOf(17))
        ...
        private static void Factor(out Graph g)
        {
            string name;
            int kind;
            Position pos;
            ...
            switch (t.kind)
            {
                ...
                case 26:
                    {
                        ...
                        var p = new Node(typ, sym, token.line);
                        g = new Graph(p);
                ...
                case 35:
                    {
                        SemText(out pos);
                        var p = new Node(Node.sem, null, 0)
                        {
                            pos = pos
                        };
                        g = new Graph(p);
                ...
                case 22:
                    {
                        Get();
                        var p = new Node(Node.any, null, 0); // p.set is set in Tab.SetupAnys
                        g = new Graph(p);
                        break;
                    }
                case 33:
                    {
                        Get();
                        var p = new Node(Node.sync, null, 0);
                        g = new Graph(p);
                        break;
                    }
        ...
        private static void Attribs(Node p)
        {
            Expect(23);
            var beg = t.pos; var col = t.col;
            while (StartOf(7))
        ...
        private static void TokenTerm(out Graph g)
        {
            Graph g2;
            TokenFactor(out g);
            while (StartOf(15))
        ...
        private static void TokenFactor(out Graph g)
        {
            ...
            if (t.kind == 1 || t.kind == 3 || t.kind == 5)
            {
                ...
                if (kind == id)
                {
                    ...
                    var p = new Node(Node.clas, null, 0)
                    {
                        val = c.n
                    }; g = new Graph(p);
                }
        ...
        private static void SynErr(int n, int line, int col)
        {
            Errors.count++;
