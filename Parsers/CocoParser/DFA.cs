...
namespace Peter.CocoParser
{
    ...
    {				// state of finite automaton
        ...
        {
            BitArray s;
            for (var a = firstAction; a != null; a = a.next)
    ...
    {			// action of finite automaton
        ...
        public void GetTargetStates(out BitArray targets, out Symbol endOf, out bool ctx)
        {
            ...
            ctx = false;
            for (var t = target; t != null; t = t.next)
            {
    ...
    {					// info about melted states
        ...
        {
            for (var m = first; m != null; m = m.next)
    ...
    {					// info about comment syntax
        ...
        private static string Str(Node p)
        {
    ...
    public class DFA
    {
        ...
        private static TextWriter trace; 	// trace output file
        ...
        private static void PutRange(BitArray s)
        {
            ...
            {
                var s1 = new BitArray(max); s1[hi[0] + 1] = true;
        ...
        private static string Char2Hex(char ch)
        {
            ...
            while (i < s.Length)
            {
                if (s[i] == '\\')
                {
                    switch (s[i + 1])
                    {
                        ...
                        default: Parser.SemErr(28); i += 2; break;
                    }
                }
                else
                {
        ...
        private static void FindUsedStates(State state, BitArray used)
        {
        ...
        {
            var newState = new State[State.lastNr + 1];
            var used = new BitArray(State.lastNr + 1);
        ...
        private static State TheState(Node p)
        {
            State state;
        ...
        private static void Step(State from, Node p, BitArray stepped)
        {
            if (p == null) return;
            stepped[p.n] = true;
            switch (p.typ)
        ...
        private static void NumberNodes(Node p, State state)
        {
        ...
        private static void FindTrans(Node p, bool start, BitArray marked)
        {
        ...
        private static Symbol MatchedDFA(string s, Symbol sym)
        {
            int i, len = s.Length;
            var weakMatch = false;
            // s has no quotes
        ...
        { // store string either as token or as literal
            var name = Unescape(sym.name.Substring(1, sym.name.Length - 2));
            if (name.IndexOf('\0') >= 0) Parser.SemErr(27);
            var matchedSym = MatchedDFA(name, sym);
            if (matchedSym == null)
        ...
        private static bool MakeUnique(State state)
        ...
        private static void MeltStates(State state)
        {
            ...
            for (var action = state.firstAction; action != null; action = action.next)
            {
                if (action.target.next != null)
                {
                    ...
                    var melt = Melted.StateWithSet(targets);
                    if (melt == null)
                    {
        ...
        private static void FillStartTab(int[] startTab)
        {
            ...
            for (var action = firstState.firstAction; action != null; action = action.next)
            {
                ...
                {
                    var s = CharClass.Set(action.sym);
        ...
        {
            int i, j;
            var startTab = new int[CharClass.charSetSize];
            var dir = System.Environment.CurrentDirectory;
            var fr = dir + "\\Scanner.frame";
            if (!File.Exists(fr))
            {
            ...
            try
            {
                var fn = dir + "\\Scanner.cs";
                if (File.Exists(fn)) File.Copy(fn, fn + ".old", true);
                var s = new FileStream(fn, FileMode.Create);
